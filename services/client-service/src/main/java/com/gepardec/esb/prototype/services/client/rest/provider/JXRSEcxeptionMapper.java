...
import com.gepardec.esb.prototype.services.client.rest.exception.RestClientExcepton;
...
public class JXRSEcxeptionMapper {
    ...
    @Provider
    @Logging
    public static class RestClientExceptionMapper implements ExceptionMapper<RestClientExcepton> {

        @Override
        public Response toResponse(RestClientExcepton exception) {
            markSpanError(exception);
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                           .header("Content-Type", MediaType.TEXT_PLAIN)
                           .entity(String.format("REST-Client caused error. exception: %s, msg: %s",
                                                 exception.getClass().getName(),
                                                 exception.getMessage())).build();
        }
    }
